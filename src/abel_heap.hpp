...
std::pair<std::vector<double>, std::vector<double>> read_conditional_probability(std::ifstream& in) {
    double y, prob;
...
std::vector<double> calculate_bins_bounds(const std::vector<double>& y, int K_BINS) {
...
std::vector<double> calculate_conditional_probability_by_bins(const std::vector<bool>& x, 
    ...
    for (int i = 0; i < n; ++i) {
        int n_bin = lower_bound(bins_bounds.begin() + 1, bins_bounds.end(), y[i]) - bins_bounds.begin() - 1;
        ++count_in_bin[n_bin][x[i]];
    }

    std::vector<double> conditional_probability_X1y(K_BINS);
    for (int i = 0; i < K_BINS; ++i) {
        conditional_probability_X1y[i] = count_in_bin[i][1] * 1. / (count_in_bin[i][0] + count_in_bin[i][1]);
    }
    return conditional_probability_X1y;
}

template<class T>
int find_nearest(const vector<T>& a, T x) {
    if (a.size() == 1) {
        return 0;
    }
...
vector<double> calculate_predictions(const vector<double>& y, 
    ...
    for (double yi : y) {
        predictions.push_back(conditional_probability[find_nearest(bins_middle, yi)]);
    }

    return predictions;
}

std::tuple<std::vector<double>, std::vector<double>, std::vector<double>> roc_curve(const std::vector<bool>& y_true, 
...
void print_conditional_probability(ostream& out, 
                                 const vector<double>& bins_bounds,
                                 const vector<double>& conditional_probability) {
...
void print_roc_curve(ostream& out,
                     const vector<double>& fpr, 
